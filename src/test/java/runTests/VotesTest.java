...
public class VotesTest {
	...
	void teste01postSucesso() throws IOException, Exception {
		...
		RequestSpecification httpRequest = RestAssured.given()
				...
				.body(BodyVotes.bodyVotes().toJSONString());
	...
	void teste02getSucesso() throws IOException, Exception {
		...
		Assert.assertEquals(js.get("value"), 1);
	...
	void teste04postCampoRequerido() throws IOException, Exception {
		...
		RequestSpecification httpRequest = RestAssured.given()
				.header("Content-Type", "application/json")
				.body(BodyVotes.bodyVotesSemValue().toJSONString());

		Response response = httpRequest.request(Method.POST);
		Assert.assertEquals(response.getStatusCode(), 400);
		JsonPath js = response.jsonPath();
		Assert.assertEquals(js.get("message"), "\"value\" is required");
